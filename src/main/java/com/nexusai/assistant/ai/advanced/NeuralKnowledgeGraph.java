...
public class NeuralKnowledgeGraph {
    ...
    public static class KnowledgeNode {
        ...
        public KnowledgeNode(long id, String label) { this.id = id; this.label = label; this.timestamp = System.currentTimeMillis(); }
    }
    ...
    public synchronized void addKnowledge(String subject, String predicate, String object, float confidence) {
        ...
        adj.computeIfAbsent(sNode.id, k -> new HashSet<>()).add(edge);
        revAdj.computeIfAbsent(oNode.id, k -> new HashSet<>()).add(edge);
        
        updateTransE(sNode.id, predicate, oNode.id, confidence);
    }

    private KnowledgeNode getOrCreateNode(String label) {
        return nodes.computeIfAbsent(label.toLowerCase().trim(), k -> {
            KnowledgeNode n = new KnowledgeNode(nodeCounter++, k);
            entityEmbeds.put(n.id, initEmbedding());
            return n;
        });
    }
    ...
    public float reason(String subject, String relation, String object) {
        KnowledgeNode s = nodes.get(subject.toLowerCase());
        KnowledgeNode o = nodes.get(object.toLowerCase());
        ...
        float dist = 0;
        for (int i = 0; i < dim; i++) dist += Math.pow(h[i] + r[i] - t[i], 2);
        return (float) Math.exp(-Math.sqrt(dist));
    }

    private float[] initEmbedding() {
        float[] e = new float[dim];
        for (int i = 0; i < dim; i++) e[i] = (float) (Math.random() * 2 - 1) * 0.1f;
        return e;
    }
}
